...
let searchingMethodUsingDFS = (root, val) => {
  if (root === null) return false;
  if (root.val === val) return true;
  const left = searchingMethodUsingDFS(root.left, val);
  const right = searchingMethodUsingDFS(root.right, val);
  return left || right;
};
let root = null;
...
console.log(
  ...
  searchingMethodUsingDFS(root, 10)
);
console.log(
  "Searching (21) using BFS in the Binary Tree =>: ",
  searchingMethodUsingDFS(root, 21)
);
console.log(
  "Searching (9) using BFS in the Binary Tree =>: ",
  searchingMethodUsingDFS(root, 9)
);
